...
class DPS(DDPM):
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            # Give condition.
            forward_model = lambda x: agem.fft_blur(x, kernel)
            scale = torch.from_numpy(self.sqrt_alphas_cumprod).to(time.device)[time].float()

            diff = measurement - forward_model(out['pred_xstart'])
            norm = torch.linalg.norm(diff)
            norm_grad = torch.autograd.grad(outputs=norm, inputs=img)[0]
            img = out['sample'] - scale * norm_grad
...
@register_sampler(name='fastem_dps')
class EMDPS(DDPM):
    def init_ker(self, ksize=33, device='cuda'):
...
class PiGDM(DDPM):
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            img = img.requires_grad_()
            out = self.p_sample(x=img, t=time, model=model)
            # Guidance is applied between [0,1]
            x0_hat = ((out['pred_xstart'] + 1)/2).clamp(0, 1)
            
            # Give condition.
            alpha_bar = self.alphas_cumprod[time[0]]
            alpha_bar_prev = self.alphas_cumprod_prev[time[0]]

            beta_prod_t = 1 - alpha_bar
            beta_prod_t_prev = 1 - alpha_bar_prev

            v = (beta_prod_t_prev / beta_prod_t) * (1 - alpha_bar / alpha_bar_prev)
            
            alpha_t = 1 / (1+v)
            r = math.sqrt(v/(v+1))

            g = (agem.deblurring_guidance(measurement, x0_hat, kernel, sigma=sigma, r=r).detach() * x0_hat).sum()
            grad = torch.autograd.grad(outputs=g, inputs=img)[0]

            img = img.detach_()
            scale = self.sqrt_alphas_cumprod[time[0]]

            img = out['sample'] + scale * grad * (r ** 2) * math.sqrt(alpha_t)
...
@register_sampler(name='fastem_pigdm')
class EMPiGDM(DDPM):
    def init_ker(self, ksize=33, device='cuda'):
        k = torch.zeros((1, 1, ksize, ksize)).to(device)
        k[0, 0, ksize//2, ksize//2] = 1
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            g = (agem.deblurring_guidance(measurement, x0_hat, k_0, sigma=sigma, r=r).detach() * x0_hat).sum()
            grad = torch.autograd.grad(outputs=g, inputs=img)[0]

            img = img.detach_()
            scale = math.sqrt(alpha_bar)
